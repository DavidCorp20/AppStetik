...
async def _hardened_current_user(
    ...
    if user.get("is_business_user") and user.get("business_id"):
        ...
        account = await server.db.users.find_one({"id": data_owner_id}, {"_id": 0, "password": 0})
        if account is None:
            raise HTTPException(status_code=401, detail="Cuenta de negocio no encontrada")

    allowed, reason = _subscription_is_valid(account)
    if not allowed:
        raise HTTPException(status_code=403, detail=reason, headers={"X-Stetik-Code": "SUBSCRIPTION_REQUIRED"})
...
def _validate_invoice_payload(payload: dict[str, Any]) -> str | None:
    ...
    if abs(client_subtotal - subtotal) > 0.01:
        return "El subtotal no coincide con los items"
    if abs(client_total - expected_total) > 0.01:
...
class ProductionHardeningMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if path.rstrip("/") == "/api/auth/upgrade" and os.getenv("STETIK_ALLOW_LEGACY_UPGRADE", "false").lower() != "true":
            return JSONResponse(status_code=410, content={"detail": "El upgrade directo está deshabilitado. La activación se realiza mediante el flujo de pago."})

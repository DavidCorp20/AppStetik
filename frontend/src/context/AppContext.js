...
export const AppProvider = ({ children }) => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      const [prodRes, estilosRes, disenosRes, gastosRes, ganRes, clientesRes, citasRes, alertasRes] = await Promise.all([
        authAxios.get(`${API}/productos`), authAxios.get(`${API}/estilos`), authAxios.get(`${API}/disenos`),
        authAxios.get(`${API}/gastos`), authAxios.get(`${API}/ganancias/config`), authAxios.get(`${API}/clientes`),
        authAxios.get(`${API}/citas`), authAxios.get(`${API}/alertas`),
      ]);
      setProductos(normalizeList(prodRes.data, ['productos', 'data']));
      setEstilos(normalizeList(estilosRes.data, ['estilos', 'data']));
      setDisenos(normalizeList(disenosRes.data, ['disenos', 'data']));
      setGastos(asObject(gastosRes.data, {}));
      setConfigGanancias(asObject(ganRes.data, null));
      setClientes(normalizeList(clientesRes.data, ['clientes', 'data']));
      setCitas(normalizeList(citasRes.data, ['citas', 'data']));
      setAlertas(normalizeList(alertasRes.data, ['alertas', 'data']));
    } catch (err) {
  ...
  const seedData = async () => { await authAxios.post(`${API}/seed`); await fetchAllData(); };
  ...
  const refreshProductos = async () => { const res = await authAxios.get(`${API}/productos`); const data = normalizeList(res.data, ['productos', 'data']); setProductos(data); return data; };
  ...
  const updateGastos = async (gastosData) => {
    ...
    const saved = asObject(res.data, gastosData);
    setGastos(saved);
    return saved;
  };
  ...
  const refreshAlertas = async () => { try { const res = await authAxios.get(`${API}/alertas`); setAlertas(normalizeList(res.data, ['alertas', 'data'])); } catch (err) { console.error('Error fetching alertas:', err); } };
  const clearData = useCallback(() => { setProductos([]); setEstilos([]); setDisenos([]); setGastos({}); setConfigGanancias(null); setClientes([]); setCitas([]); setAlertas([]); setError(null); }, []);
